...
int version = 25;  //22; //last good version
...
#include <Arduino.h> 
#include <esp_dmx.h>  //v3.1.0 Mitch Weisbrod
...
*/
#include <Adafruit_GFX.h> //1.1.10
#include <Adafruit_SSD1306.h> //2.5.11

//Touch Buttons
#include "Adafruit_MPR121.h" //1.1.3
...
void setup() {
 ...
 blink(1);
  ...
  if (!cap.begin(0x5A)) {
    Serial.println("M2 MPR121 not found, check wiring?");
    Serial.println("(No Keypad)");
    Serial.println("Check Wiring");
    Serial.println("Error 102077");
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
  display.println("M3 MPR121 not found");
  display.println("(No Keypad)");
  display.println("Check Wiring");
  display.println("Error 102077");
  display.display();
  delay(4000);
  //  while (1)
  //    ;
...
//See if the Rpi wants control Pin14 = hi
   ...
   /* 
   Display Channel Data sent out to DMX
