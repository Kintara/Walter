...
int version = 22;  //20; //last good version
#include <EEPROM.h>
...
#define PIN_LED 2
#define GPIO_Pin_Snow 12  //To Rpi Zero2 W to trigger Snowing sequence > CAT pin 3
//#define GPIO_Pin_Stopping 13 //To Rpi Zero2 W to trigger Stopping sequence > CAT pin 4
//#define GPIO_Pin_Starting 14 //To Rpi Zero2 W to trigger Starting soon sequence > CAT pin 5
#define GPIO_Pin_AwaitCommandFromRPi 13        //From Rpi Zero2 W to stop the blower and await Snow Command from RPi  > CAT pin 4
#define GPIO_Pin_ProduceSnowCommandFromRpi 14  //From Rpi Zero2 W : Start Snow! command > CAT pin 5
int RPiStatus = 0;
/* NO LONGER USED
int CH_ValueUP = 25;       //{ 25, false };
int CH_ValueDown = 26;     //{ 26, false };
int DurationUp = 32;       // { 32, false };
int DurationDown = 33;     // { 33, false };
int IntervalUp = 18;       //{ 18, false };
int IntervalDown = 15;     //{ 15, false };
int CH_SelectButton = 12;  //{ 12, false };
*/
...
void setup() {
  ...
  if (ssid == "" || password == "") {
    ...
    WiFi.softAP("Snow", "");
    Serial.println("AP Mode Snow &  IP=192.168.1.1");
    delay(200);
    WiFi.softAPConfig(PageIP, gateway, subnet);
  } else {
    // Connect to Wi-Fi
    Serial.println();
    Serial.print("Use EEPROM Values");
    Serial.println(ssid);
  ...
  //pinMode(23, INPUT_PULLUP); //possible from RPI GPIO pin to start blower
  //Set up Pin modes and ISRs
  /*pinMode(CH_ValueUP, INPUT_PULLUP);
  attachInterrupt(CH_ValueUP, CH_ValueUPISR, FALLING);

  pinMode(CH_ValueDown, INPUT_PULLUP);
  attachInterrupt(CH_ValueDown, CH_ValueDownISR, FALLING);

  pinMode(DurationUp, INPUT_PULLUP);
  attachInterrupt(DurationUp, DurationUpISR, FALLING);

  pinMode(DurationDown, INPUT_PULLUP);
  attachInterrupt(DurationDown, DurationDownISR, FALLING);

  pinMode(IntervalUp, INPUT_PULLUP);
  attachInterrupt(IntervalUp, IntervalUpISR, FALLING);

  pinMode(IntervalDown, INPUT_PULLUP);
  attachInterrupt(IntervalDown, IntervalDownISR, FALLING);

  pinMode(CH_SelectButton, INPUT_PULLUP);
  attachInterrupt(CH_SelectButton, CH_SelectISR, FALLING);
*/
  for (int x; 0; 255) {
    data[x] = 0;
  }
...
//See if the Rpi wants control Pin14 = hi
    if (digitalRead(GPIO_Pin_AwaitCommandFromRPi) == 1 ) {
      ...
      timingMode = 4;
      //0=Stop 1=interval 2= Duration ; 3= Full Snow ;4=RPi Control
      RPiMode ="Primed";
